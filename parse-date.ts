const MSG_INVALID_DATE = 'Date is not valid!'
...
export const parseDate = (s: string, d: Date) => {
	...
	if (s === 'yesterday') {
		...
		const interval = s.split('next')[1].trim()
		if (MonthMap[interval] !== undefined) {
			...
			newDate.setDate(
				newDate.getDate() - newDate.getDay() + 7 + DayMap[interval],
			)
		}
		return newDate
	} else if (s.indexOf('this') > -1) {
		const interval = s.split('this')[1].trim()
		newDate.setDate(newDate.getDate() - newDate.getDay() + DayMap[interval])
		return newDate
	} else if (s.indexOf('last') > -1) {
		const interval = s.split('last')[1].trim()
		if (MonthMap[interval] !== undefined) {
			...
			newDate.setDate(
				newDate.getDate() - newDate.getDay() - 8 + DayMap[interval],
			)
		}
		return newDate
	}
	...
	if (s.indexOf('from') > -1) {
		const [timePeriod, timeStart] = s.split('from').map((pt) => pt.trim())
		const periods: string[] = timePeriod.split(' ')
		const period = periods.pop()
		let interval: any = periods
			.join('-')
			.replace(
				new RegExp(Object.keys(NumberMap).join('|'), 'g'),
				(match: string) => NumberMap[match],
			)
			.replace(/000|00|0\-/g, '')
			.replace(/\-/g, '')
		if (!interval || !period) {
